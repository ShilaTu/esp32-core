 ...
 */
#define CHANNEL_INIT(name, identifier, ctx, flags, callback) \
    { "" identifier "",                                      \
      LIST_HEAD_INIT(name.same),                             \
      LIST_HEAD_INIT(name.unique),                           \
      ctx, flags,                                            \
      callback }
 ...
 */
#define CHANNEL(name, identifier, ctx, flags,                           \
                callback) Channel name = CHANNEL_INIT(name,             \
                                                      "" identifier "", \
                                                      ctx,              \
                                                      flags,            \
                                                      callback)
...
EXPORT
void
channel_internal_init(Channel         *ch,
                      const char      *identifier,
                      void            *ctx,
                      const BaseType_t flags,
                      Channel_callback callback)
...
EXPORT
void
channel_internal_register(Channel *ch)
{
    Channel *curr;
...
EXPORT
void
channel_internal_unregister(Channel *ch)
{
    ...
    if (is_unique && is_same)
    {
        ...
        Channel *next_uniq
            = list_entry(ch->unique.next, struct channel, unique);
        list_del(&ch->same);
        INIT_LIST_HEAD(&ch->same);
        list_del(&ch->unique);
        INIT_LIST_HEAD(&ch->unique);
...
channel_internal_resetRoot(void)
{
